...
2. **Migrations** — **39** numbered files under `supabase/migrations/` (there is **no** `021_*`; the sequence jumps from `020_*` to `022_*`). They are the source of truth: FDC nutrition (`026+`), grocery tables (`027`), comments (`028`), `fdc_candidates` (`029`), legacy **`ingredients` removal (`030`)**, archive/alignment (`031`–`033`), FDC hourly quota (`034`–`035`), grocery item category (`036`), folder cover image URL (`037`), soft delete columns (`038`), trash purge + cron (`039`), and **`040` drops `legacy_recipes_archive` after offline export**. Apply via CLI or MCP as above. The SQL Editor is for one-off debugging only.
