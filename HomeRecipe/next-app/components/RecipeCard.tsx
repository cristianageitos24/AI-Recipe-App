...
export function RecipeCard({ recipeData }: RecipeCardProps) {
  ...
  const [folders, setFolders] = useState<{ folders?: string[] } | null>(null);
  ...
  async function handleEtcShowOptions(e: React.MouseEvent) {
    e.stopPropagation();
    setIsFolderBtnClicked(!isFolderBtnClicked);
    if (!isFolderBtnClicked) {
      const res = await getFolders();
      if (res.data) setFolders({ folders: res.data.folders });
    }
  }

  async function handleAddToFolder(folderName: string) {
    await addRecipeToFolder(folderName, recipeData.recipe_id);
    ...
    >
        ...
        <div className="options-btn-bkg">
          ...
          <div className="save-to-folder-content">
            ...
            >
              ...
              {isFolderBtnClicked && folders?.folders && (
                <div className="save-to-folder-options">
                  {folders.folders.map((folderName) => (
                    <p key={folderName} onClick={() => handleAddToFolder(folderName)}>
                      {folderName}
                    </p>
                  ))}
                </div>
      ...
      <div className="recipe-card-information-box">
        <div className="recipe-card-labels">
          ...
          <div className="small-labels">
            <p><span>{recipeData.calories}</span> calories</p>
            <p>
