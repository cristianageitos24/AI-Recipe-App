...
export const RECIPE_LIST_COLUMNS =
  "id, recipe_id, recipe_label, calories, cuisine_type, meal_type, time_in_minutes, image_url, website_url, created_at, deleted_at, user_id, expires_at, recipe_nutrition(energy_kcal, protein_g, fat_g, carb_g, nutrition_source, servings)" as const;
...
export const RECIPE_FOLDER_COLUMNS =
  "id, recipe_id, recipe_label, calories, cuisine_type, meal_type, time_in_minutes, image_url, website_url, ingredient_lines, steps, created_at, deleted_at, user_id, expires_at, recipe_nutrition(energy_kcal, protein_g, fat_g, carb_g, nutrition_source, servings)" as const;
...
/**
 * Prefer synced `recipe_nutrition.energy_kcal` when present and &gt; 0;
 * otherwise fall back to legacy `recipes.calories` (avoids showing 0 when calories column has a value).
 */
export function recipeDisplayEnergyKcal(
  ...
  const legacy = Number(row.calories);
